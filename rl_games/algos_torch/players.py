...
class PpoAdvPlayerContinuous(BasePlayer):
    def __init__(self, params):
        BasePlayer.__init__(self, params)
        self.network = self.config['network']
        self.actions_num = self.action_space.shape[0] 
        self.actions_low = torch.from_numpy(self.action_space.low.copy()).float().to(self.device)
        self.actions_high = torch.from_numpy(self.action_space.high.copy()).float().to(self.device)
        adv_actions_low = torch.ones(6, device=self.device).float() * self.actions_low[0]
        adv_actions_high = torch.ones(6, device=self.device).float() * self.actions_high[0]
        self.actions_low = torch.cat([self.actions_low, adv_actions_low])
        self.actions_high = torch.cat([self.actions_high, adv_actions_high])
        ...
        self.model.to(self.device)
        self.model.eval()
        self.adv_model = self.network.build(adv_config)
    ...
    def get_action(self, obs, is_deterministic = False):
        ...
        mu = res_dict['mus']
        action = res_dict['actions']
        adv_mu = adv_res_dict['mus']
        adv_action = adv_res_dict['actions']
        self.states = res_dict['rnn_states']
        if is_deterministic:
            current_action = mu
            current_adv_action = adv_mu
        else:
            current_action = action
            current_adv_action = adv_action
        if self.has_batch_dimension == False:
            current_action = torch.squeeze(current_action.detach())
            current_adv_action = torch.squeeze(current_adv_action.detach())
        action = torch.cat([current_action, current_adv_action], dim=-1)
        if self.clip_actions:
            return rescale_actions(self.actions_low, self.actions_high, torch.clamp(action, -1.0, 1.0))
        else:
            return current_action
